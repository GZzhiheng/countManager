...
$(document).ready(function(){
    var table = $('#alarmTable').dataTable({
        ...
        "ajax": {
            "url": window.host+"/alert/alertTriggerRecord/list4Eliminate.json",
            "type": "POST",
        ...
        "drawCallback":function(){//表格tabale渲染完的回调函数，调用
            ...
            $trs.each(function(i){
                ...
                {
                    case 2:
                        $(this).parents("tr").find("td .alarmSeq").prepend('<i class="glyphicon glyphicon-bell red" style="position:absolute;left:0px;"></i>');break;
                }
            });
        },
        "columns": [
            ...
            {"data": "accountName", searcher: true,orderable: true,
                "render": function (data, type, full) {
                    // return "<a href='#"+window.host +"/clientmgr/client/update?clientId="+full.clientId+"'>"+full.accountName+"</a>";
                    return full.accountName;
                }
            },
            {"data": "accountId", searcher: true,orderable: true,
                "render": function (data, type, full) {
                    // return "<a href='#"+window.host +"/clientmgr/client/update?clientId="+full.clientId+"'>"+full.accountId+"</a>";
                    return full.accountId;
                }
            },
            {"data": "settingDetailName", searcher: false, orderable: true},
            {"data": "settingDetailStatus",
                searcher: false,
                orderable: true,
                "render": function (data, type, full) {
                    var settingDetailStatusCN = "";
                    switch(full.settingDetailStatus)
                    {
                        case "0": settingDetailStatusCN="<span class='alarmLevel' data-status='0' style='color:green'>关注</span>";break;
                        case "1": settingDetailStatusCN="<span class='alarmLevel' data-status='1' style='color:orange'>重要</span>";break;
                        case "2": settingDetailStatusCN="<span class='alarmLevel font-bold' data-status='2' style='color:red'>紧急</span>";break;
                    }
                    return settingDetailStatusCN;
                }
            },
            {"data": "firstTriggerDate",
                ...
                orderable: false,
                "render": function (data, type, full) {
                    return cf.formatDate(data,"yyyy-MM-dd")
                }
            },
            {"data": "eliminateAt",
                searcher: false,
            ...
            },
            {
                ...
                orderable: false,
                "render": function (data, type, full) {
                    return "<a href='#"+window.host +"/alert/alertTriggerRecord/update?isEliminate=true&id="+full.id+"'>详情</a>";
                }
            }
        ]
    });
    $('#alarmTable').on('init.dt', function (e, cfg) {
    ...
    $("#clientMgrId").change(function () {
        loadClients("#clientMgrId", "#clientId");
    });
