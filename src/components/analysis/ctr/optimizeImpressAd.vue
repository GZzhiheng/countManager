<template>
  ...
  <div>
    <div class="clearfix pad-lt-rt-20 position-relative title-box">
      <div class="pull-left table-title-left-width">
        ...
        <div class="form-span-color table-blod-title">
          <span>过去7天里，以下为展示次数多且点击率低于</span>
          <span class="suggest-data-color">0.1%</span>
          <span>的展示位置，这些展示位置可能与您的业务不想关，并且会降低广告的整体点击率，建议排除这些展示位置。</span>
        </div>
      </div>
      <div class="pull-left table-title-right-width absolute-right-bottom">
        ...
        <div :class="selectId.length > 0 ? '': 'btn-mask-box'">
          ...
          <a class="btn normal-blod-ok-btn mask-normal-blod-ok-btn height-28 vertical-initial pull-right mr-20" @click="addDialog">排除</a>
      ...
      <div class="el-table el-table--fit el-table--border el-table--fluid-height el-table--enable-row-hover el-table--enable-row-transition" style="width: 100%;">
        <div class="el-table__header-wrapper">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__header position-relative" style="width: 100%;">
            ...
            <tr>
              ...
              <th class="el-table_1_column_23 is-leaf is-sortable" :class="order[0]" style="width: 30%" @click="sortMethod({prop: 'criteria', order: order[0], index: 0})" >
                <div class="cell">展示位置<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_24 is-leaf is-sortable" :class="order[1]" style="width: 12%" @click="sortMethod({prop: 'groupName', order: order[1], index: 1})">
                <div class="cell">广告组<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_25 is-leaf is-sortable" :class="order[2]" style="width: 12%" @click="sortMethod({prop: 'campaignName', order: order[2], index: 2})">
              ...
              <th class="el-table_1_column_28 is-leaf is-sortable" :class="order[4]" style="width: 8%" @click="sortMethod({prop: 'impressions', order: order[4], index: 4})">
                <div class="cell">展示次数<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_30 is-leaf is-sortable" :class="order[5]" style="width: 7%" @click="sortMethod({prop: 'ctr', order: order[5], index: 5})">
                <div class="cell">点击率<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_30 is-leaf is-sortable" :class="order[6]" style="width: 8%" @click="sortMethod({prop: 'conversions', order: order[6], index: 6})">
                <div class="cell">转化次数<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_29 is-leaf is-sortable" :class="order[7]" style="width: 10%" @click="sortMethod({prop: 'cost', order: order[7], index: 7})">
        ...
        <div class="el-table__body-wrapper">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width: 100%;min-width: 1056px">
            <tbody>
            <tr class="el-table__row" v-if="impressAd.datas.length > 0" v-for="item in impressAd.datas">
              <td class="el-table_1_column_22 el-table-column--selection" style="width: 5%">
                <div class="cell">
                  <div class="checkbox">
                    <input :id="item.id" :value="item.id" v-model="selectId" type="checkbox" @click="select({id: [item.id], isSelect: item.isSelect, type: 'CLICK_RATE_DISPLAY'})">
                    <label :for="item.id"><span></span><i class="fa fa-check"></i></label>
                  </div>
                </div>
              </td>
              <td class="el-table_1_column_23" style="width: 30%"><div class="cell"><a :href="item.criteria" title="item.criteria" target="_blank" class="ellipsis aHoverLine">{{item.criteria || module.tableEmptyPlaceholder}}</a></div></td>
              <td class="el-table_1_column_24" style="width: 12%"><div class="cell">{{item.groupName || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_25" style="width: 12%"><div class="cell">{{item.campaignName || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_26" style="width: 8%"><div class="cell">{{item.clicks || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_27" style="width: 8%"><div class="cell">{{item.impressions || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_28" style="width: 7%"><div class="cell">{{item.ctr || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_29" style="width: 8%"><div class="cell">{{item.conversions || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_30 text-right" style="width: 10%"><div class="cell">￥{{$toFixed(item.cost) || module.tableEmptyPlaceholder}}</div></td>
              <td class="gutter"></td>
            </tr>
            <tr v-if="impressAd.datas.length === 0" class="emptyTr text-center"><td colspan="9">{{module.tableNullData}}</td></tr>
            </tbody>
          </table>
        </div>
    ...
    <!--page-->
    <pagination
      :request="impressAd.sortStatus"
      :total="impressAd.listTotal"
      :pageSize="query.pageSize"
      :dispatch="'getCtrOpImpressAdList'"
    ></pagination>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      impressAd () {
        return this.$store.state.ctr.optimize.impressAd
      },
      list () {
        return this.$store.state.ctr.optimize.impressAd.list
      },
      selectId: {   // 选中id数组
        get () {
          return this.$store.state.ctr.optimize.impressAd.selectId
        },
        set (val) {
          this.$store.commit('GET_CTR_OP_IMPRESS_AD_SELECT_ID', val)
        }
      },
      allSelectStatus: { // 全选状态
        get () {
          return this.$store.state.ctr.optimize.impressAd.allSelectStatus
        },
        set (val) {
          this.$store.commit('GET_CTR_OP_IMPRESS_AD_SELECT', val)
        }
      }
    },
    methods: {
      ...
      checkedAll () { // 全选
        ...
        if (!_this.allSelectStatus) { // 实现全选
          _this.selectId = []
          _this.impressAd.datas.forEach(function (item) {    // 列表全选中，获取全部ID
        ...
        var requestId = []
        _this.impressAd.datas.forEach(function (item) {    // 不管选不选中，都要获取全部ID发送后端
          requestId.push(item.id)
        })

        let request = {
          id: requestId,
          type: _this.impressAd.datas[0].type,
      ...
      sortMethod (argus) { // 排序
        ...
        this.$store.commit('GET_CTR_OP_IMPRESS_AD_SORT', argus)
      },
      addDialog () {
        var _this = this
        _this.$confirm('排除后不再展示，是否确定排除！', '', {
          cancelButtonText: '取消',
          ...
          _this.$store.dispatch('getCtrOpImpressAdList', {pageSize: _this.query.pageSize, pageNum: _this.query.pageNum}) // 刷新列表
          _this.$store.dispatch('getCtrOpImpressExcludeDialogOk')
        }).catch(function () {  // 取消方法
        })
      }
    }
