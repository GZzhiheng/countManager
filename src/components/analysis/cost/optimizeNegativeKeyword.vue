    ...
    <div class="clearfix pad-lt-rt-20 position-relative title-box">
      <div class="pull-left table-title-left-width">
        ...
        <div class="form-span-color table-blod-title">
          <span>过去30天里，有</span>
          <span class="suggest-data-color">{{nk.list.rowCount}}</span>
          <span>个搜索词</span>
          <span>触发了您的关键字并产生了费用，这些字词可能与你的业务无关，共浪费金额</span>
          <span class="suggest-data-color">{{$toFixed(nk.totalWastCost) || module.emptyPlaceholder}}</span>
          <span>元，立即添加为否定关键字，未来一年可节省</span>
          <span class="suggest-data-color">{{$toFixed(nk.totalSaveCost) || module.emptyPlaceholder}}</span>
          <span>元</span>
        </div>
      </div>
    ...
    <div class="col-xs-12 table-shadow table-title-top mb-20">
      <div class="el-table el-table--fit el-table--border el-table--fluid-height el-table--enable-row-hover el-table--enable-row-transition" style="width: 100%;">
          ...
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__header position-relative" style="width: 100%;min-width: 1074px">
            ...
            <tr>
              <th class="el-table_1_column_22 el-table-column--selection is-leaf" style="width: 5%">
                <div class="cell">
                  <div class="checkbox">
                    <input id="allSelectNk" v-model="allSelectStatus" @click="checkedAll" type="checkbox">
                    <label for="allSelectNk"><span></span><i class="fa fa-check"></i></label>
                  </div>
                  </label>
                </div>
              </th>
              <th class="el-table_1_column_23 is-leaf is-sortable" :class="order[0]" style="width: 24%" @click="sortMethod({prop: 'criteria', order: order[0], index: 0})" >
                <div class="cell">待否定关键字<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              ...
              <th class="el-table_1_column_27 is-leaf is-sortable" :class="order[4]" style="width: 8%" @click="sortMethod({prop: 'impressions', order: order[4], index: 4})">
              ...
              <th class="el-table_1_column_29 is-leaf is-sortable" :class="order[6]" style="width: 10%" @click="sortMethod({prop: 'cost', order: order[6], index: 6})">
                <div class="cell">总消费<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
              <th class="el-table_1_column_30 is-leaf is-sortable" :class="order[7]" style="width: 13%" @click="sortMethod({prop: 'expectWastCost', order: order[7], index: 7})">
                <div class="cell">预计未来一年浪费<span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span></div>
              </th>
        ...
        <div class="el-table__body-wrapper">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width: 100%;min-width: 1057px">
            <tbody>
            <tr class="el-table__row" v-if="nk.datas.length > 0" v-for="item in nk.datas">
              ...
              <td class="el-table_1_column_23" style="width: 24%"><div class="cell" v-html="item.criteria"></div></td>
              ...
              <td class="el-table_1_column_29 text-right" style="width: 10%"><div class="cell">￥{{$toFixed(item.cost) || module.tableEmptyPlaceholder}}</div></td>
              <td class="el-table_1_column_30 text-right" style="width: 13%"><div class="cell">￥{{$toFixed(item.expectWastCost) || module.tableEmptyPlaceholder}}</div></td>
              <td class="gutter"></td>
            </tr>
            <tr v-if="nk.datas.length === 0" class="emptyTr text-center"><td colspan="8">{{module.tableNullData}}</td></tr>
            </tbody>
          </table>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      openDialog () {
        this.$store.commit('GET_NK_OPTIMIZE_DIALOG', true)
      },
      scrollTbody () {
